...
- **Contoh (aksi durable):** [examples/tls/webtransport_tasks.zig](examples/tls/webtransport_tasks.zig): halaman lewat HTTPS/1.1 di TCP pada 127.0.0.1:9444, session lewat HTTP/3 di QUIC pada 127.0.0.1:9444: dibangun sebagai `zig build example-webtransport_tasks`: halaman mengirim typed form event dengan idempotency key lewat stream reliabel, server memvalidasi dan mengotorisasi, satu transaksi menulis task dan job-nya, worker me-lease dan menjalankan job, satu transaksi completion menulis update task, penyelesaian job, dan baris outbox, dispatcher mempublikasikannya, dan view yang terotorisasi mengirim patch berevisi. Datagram hanya membawa typing hint dari halaman, tidak pernah sebuah mutasi.
...
- **Suite acceptance:** `zig build test-durable` menjalankan tujuh skenario terhadap PostgreSQL nyata (`-Ddurable-dsn=`, database terpisah memang disengaja: worker me-lease job siap paling tua milik tenant mana pun, jadi server yang hidup dan berbagi tabel akan menyelesaikan job milik suite). Submission tidak valid atau tidak terotorisasi tidak menyimpan apa pun; transaksi yang tidak pernah commit tidak menyimpan satu pun sisipan; submission ganda menghasilkan satu task dan satu job; job milik worker yang mati dipulihkan setelah lease-nya kedaluwarsa dengan `attempts` bertambah; crash setelah commit tetapi sebelum publikasi tetap terkirim; event yang diputar ulang tidak menggeser view; dan view yang restart membangun ulang state, beserta revisinya, dari database.
  Distribusi per jenis suntingan kini terukur. **Kondisi mantap: 3,3 s**: build ~2,0 s, restart server ~82 ms, dan verifikasi browser itu sendiri ~1,21 s, yang bertahan di 1209-1214 ms untuk setiap jenis yang berjalan bersih. Dengan build yang sepenuhnya ter-cache, loop-nya **1,5 s** (build 220 ms), di bawah ambang 2-3 s yang jadi acuan pengukuran ini. Tiga dari sembilan iterasi menunjukkan verifikasi ~16,0 s alih-alih 1,2 s; penyebabnya belum diketahui, dan harness-nya kini menyimpan log server dari iterasi yang lebih lambat dari 5 s supaya jalannya berikutnya bisa menjelaskannya. Flag dan mode-nya disebut bersama angkanya: `--fresh-browser` memulai browser per iterasi, dan waktu start browser ada di dalam totalnya.
...
- **`docs/hld-grpc-en.md`**, **`hld-proxy-en.md`** (dan -id) diperbarui untuk TLS native.
...
- Koreksi dokumentasi di `docs/` dan README:
    ...
    - `zix.Tcp`: dokumentasi memakai `max_msg_len`, field sebenarnya adalah `max_recv_buf`. `docs/lld-tcp-en.md` / `docs/lld-tcp-id.md` juga sebelumnya tidak memuat model dispatch `.EPOLL` / `.URING` sama sekali.
...
- SSE / streaming melalui TLS (ADR-054):
    - `zix.Http` dan `zix.Http1` melayani Server-Sent Events melalui TLS di jalur thread-per-koneksi (`.ASYNC` / `.POOL` / `.MIXED`). Stream sink per-koneksi (`TlsStreamSink`, type-erased atas koneksi TLS 1.3 / 1.2 hidup) mengenkripsi satu TLS record per write dan mengirimnya seketika, menggantikan buffered capture hanya ketika handler memilih streaming. `writeAllFD` memeriksa buffered sink dulu, lalu stream sink, jadi response normal menjaga jalur cepat ter-buffer tetap utuh.
...
- gRPC over TLS dan terminator h2-over-TLS bersama:
    ...
    - Docs `hld-grpc`, `hld-tls`, `lld-tls`, dan `hld-proxy` (en dan -id) diperbarui untuk gRPC TLS native.
