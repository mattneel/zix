...
- **Development-loop benchmark:** [scripts/dev_loop_bench.py](scripts/dev_loop_bench.py) measures a source edit through to a *visible browser change* — build, server restart, reload, reconnection, and the browser's own report — for three edits: a handler change, a render change, and a change to the shared slice's `Task` type, which rebuilds the module and every consumer with it. The page serves its build token and its own mark into the DOM and, with `?devloop`, polls a version hash of exactly what the server serves, reloads when that changes, then proves the path again (dial, subscribe, durable action, completion patch) and only then reports through `/verified`. One measured iteration here (WSL2, Debug, warm `.zig-cache`, one Chromium kept open so an iteration pays reload rather than launch): **2.33 s total — build 1.95 s, server restart 0.14 s, reload plus verification 0.24 s**. The per-kind distribution is **not yet measured**, and the harness says so instead of inventing a number: the automated matrix stalls in this environment because a page reload cannot reliably re-dial an origin Chrome was told to force QUIC on, and this repo's scripts report that as a failure. The measurement mechanism itself is verified — the browser's report lands in the server log for the edited build — so what is missing is repetition, not the measurement.
- **Benchmark:** [scripts/bench_durable_tasks.py](scripts/bench_durable_tasks.py) runs the slice as the workload and measures the two numbers that matter — submit to commit, and submit to the completion patch — over a WebTransport session. Measured here (WSL2, local loopback, Debug build, one session sequentially, 10 ms poll, the demo's 40 ms simulated job): 200 actions, **commit p50 3.3 ms / p99 4.4 ms**, **end-to-end p50 65.5 ms / p99 88.1 ms / max 89.1 ms**, 14.1 actions/s. The subsecond target is therefore measured with roughly an order of magnitude of headroom, and the numbers are reproducible with the script and the example.
...
#### Durable slice: a worker whose lease expired cannot complete the job another took

    - Fixed: completion was unconditional. It updated the task and the job by id, so a worker that stalled past its lease overwrote the result of the attempt that owned the job by then: lease expiry was a *duplicate execution* rather than a recovery, and the slower of two workers decided the result. `Store.complete` now checks ownership inside the same transaction as the writes it guards, against `attempts` — the token every lease bumps — and a stale attempt gets `.not_owner`: no task update, no job completion, no outbox row, so no event reaches a view. `Worker` counts those drops in `lost` instead of counting them as done.
    - Covered by `durable tasks: a worker whose lease expired cannot complete the job another worker holds`: takeover, a refused completion with the task and the outbox unchanged, the owner's completion, and a refused double completion by the same attempt.

#### WebTransport over HTTP/3: a stream's acknowledged prefix is retired, so a long-lived patch stream does not stall

    - Fixed: a WebTransport data stream stalled once its buffer filled, with every byte acknowledged. `SendSide.noteSent` merges a contiguous send into the previous outstanding range, while an acknowledgement arrives per packet — a tail of that merged range — and `noteAcked` only retired a range the acknowledgement covered whole. The prefix therefore never freed, `acked` stayed at zero, the 16 KiB stream buffer filled, and the stream reported nothing more to send: a view streaming durable patches stalled after roughly 16 KiB while the peer had confirmed all of it. `noteAcked` now trims the acknowledged head of a partly covered range, so per-packet acknowledgements drain the buffer as the stream advances. Covered by `zix webtransport: 4.1 a merged send range frees its prefix from per-packet acknowledgements`.
    - Found by the durable slice's benchmark, which is the workload that made it visible: 200 sequential CreateTask actions on one session stalled at about request 34 before the fix, and complete with p99 88.1 ms after it. The echo example could not show it — it answers with a few dozen bytes, which never fills a stream buffer.
    - The example's own write path learned the same lesson: a stream write returns how much it took, so a view now buffers what the engine would not accept and resumes it on the next poll, and advances its cursor only for lines the engine took whole. A patch the outbox has marked published is never dropped.

#### WebTransport over HTTP/3: one allocator for every server-initiated stream id

    - Fixed: the server control stream was written from a bare literal on all three prologue paths while WebTransport kept its own counters, so the reservation was a comment rather than a fact. Every server-initiated stream — the control stream first, then any QPACK or WebTransport stream this engine opens — now takes its id from one per-connection allocator (`Connection.takeServerStreamId`), which is what makes the invariant "all server-initiated streams have unique ids within a connection" structural instead of contingent. Covered by `zix http3: every server-initiated stream id is unique, control stream included`, a test the old code fails: it pins the control stream's id, which nothing did before.

#### WebTransport over HTTP/3: the handshake is confirmed when the client's Finished arrives

    - Fixed: a browser could not open a session at all. A client is entitled to wait for the handshake to be confirmed before it sends any 1-RTT data, and Chromium does: it held its SETTINGS, its CONNECT, and every request until `HANDSHAKE_DONE` arrived, while the engine only wrote the confirmation when a 1-RTT packet arrived — a deadlock that aioquic and the in-tree client never exposed, because both send 1-RTT early. The server now verifies the client's Finished and leaves its one-time 1-RTT prologue (`HANDSHAKE_DONE`, then the control stream's SETTINGS) in the same turn, which is also the earliest moment RFC 9000 17.2.1 permits it.
    - Fixed: the client Finished was decrypted and discarded, so the handshake was never complete on the server side. Handshake-level CRYPTO bytes are now reassembled and the Finished is checked against the client handshake-traffic secret over the transcript through the server Finished (RFC 8446 4.4.4). A Finished that does not verify leaves the handshake unconfirmed: no confirmation, no session, and the connection idles out, so a peer that never proved key possession never reaches the request path. Covered by `zix http3: a client Finished verifies against the client handshake secret and the server-Finished transcript`.
    - Fixed: a Handshake-level CONNECTION_CLOSE (a client rejecting the certificate or a transport parameter) was decrypted and dropped, so the connection simply went quiet. It is now logged at WARN with the peer's alert code and description, which is where an interop failure names itself.
    - Fixed: the first server-opened unidirectional WebTransport stream was handed the id 3, which is the server's own HTTP/3 control stream: every unidirectional stream a session opened appended its bytes to the control stream instead, corrupting it for the peer (a client saw the stream type 0x54 and a banner tacked onto its SETTINGS). Unidirectional ids now start at 7, the first one the control stream leaves free. Covered by `zix http3: a server's WebTransport stream ids skip the control stream (RFC 9114 6.2.1)`.
    - Fixed: the runner's HTTP/3 client expected that banner on stream 3, which is what let the collision above pass its own test. The client now expects the protocol-correct id, so the runner fails if the engine ever reuses the control stream again.
    - Docs: the HLDs (both languages) gain a "Handshake and Confirmation" section, a security bullet on the confirmation rule, the browser demo in the examples table, and a "Driving the browser demo" walkthrough with the Chromium certificate flags; the changelog's New Features gains the demo and the interop harness.
